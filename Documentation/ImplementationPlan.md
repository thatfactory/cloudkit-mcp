...
| Release-acceptance profile payload policy | Current profiles are metadata-only: allowed record types, queryable fields, and readable payload fields are empty | A zero-filter live query needs an operator-supplied privacy-safe record type and explicit external startup-policy amendment; filtered date/number/value verification additionally needs an indexed field and safe value; infer and commit no private selector |
