# CloudKit MCP Project Instructions

## Context

CloudKit MCP is a local, npm-distributed, read-only stdio MCP server for inspecting account-relative CloudKit databases, records, zones, sharing, subscriptions, and change state. [Documentation/ImplementationPlan.md](Documentation/ImplementationPlan.md) is the single implementation and evidence plan.
...
| Build and package tooling | `scripts/` |
| Durable documentation | `Documentation/` |

## Stack

Node.js 24 or newer, strict TypeScript, ESM, and stdio MCP transport. Runtime network access is restricted to the closed CloudKit Web Services operation registry and exact Apple origin. Ordinary tests are credential-free and network-blocked.

## Local specialization

- The project is TypeScript rather than Swift. Repository-owned Node/TypeScript scripts are the documented automation exception required by the npm package; do not add Swift runtime dependencies.
- Runtime observability uses one repository-owned structured JSON event gateway on stderr rather than AppLogger, because AppLogger is a Swift-only package. Preserve the same privacy and lifecycle requirements.
- Stdout is reserved exclusively for MCP protocol traffic while serving.
- Never add CloudKit write operations, browser-cookie extraction, generic request tools, or hidden credential fallback paths.
