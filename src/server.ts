...
export function createServer(service: DiagnosticService): McpServer {
  ...
  register(server, "get_database_changes", "Read bounded database change state from an explicit beginning or process-bound cursor.", { view: viewSchema, start: changeStartSchema }, async ({ view, start }) => service.getDatabaseChanges(view, start as ChangeStart));
  register(server, "get_zone_changes", "Read bounded custom-zone changes and tombstones from an explicit beginning or process-bound cursor.", { view: viewSchema, zone: zoneSchema, start: changeStartSchema }, async ({ view, zone, start }) => service.getZoneChanges(view, zone, start as ChangeStart));
