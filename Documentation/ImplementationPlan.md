...
**Exit gate:** approved public reads work against fixtures and a separately authorized synthetic live target; every non-read operation is unreachable. Verify signing live before claiming live-verified server-key support.
...
- [x] Live server-key signing is retained as an explicit limitation and is not required for the proven web-user MVP path.
...
**PR 08B: changes.** Implement database and zone-change tools with explicit discriminated starts: an issued cursor or a deliberately requested beginning scan. Do not advertise a current-baseline operation until that distinct contract is verified. Each start has a distinct `coverage` description; do not equate a current baseline with a full history audit. The documented Web Services zone-change endpoint applies to custom zones and supports private/shared scope; use the current documented change endpoints rather than deprecated `records/changes`. [A6, A7, A12]
...
- [x] Privacy-safe release-candidate live acceptance complete for the enabled metadata-only profile surface; policy-disabled and separately unverified operations remain explicit limitations.
- [x] Human-gated account-switch, expiry, server-key, and additional exact-selector cases are evidenced at the applicable level or retained as explicit `0.1.0` limitations.
...
- [x] Final README and public capability contracts reconcile the accepted metadata-only surface with retained live and human-gated limitations.
- [x] Package-content and external-install gates complete.
- [x] Release-path byte identity and offline preflight gates complete.
- [x] Final repository governance and evidence reconciliation complete.
- [ ] Final release-candidate exact-head review complete and recorded externally.
...
| Normal participant-account authentication | Verified live on 2026-09-14 with the consenting invited participant account, without assuming developer-team enrollment | Retain ordinary-participant evidence; deliberate account switching remains human-gated and synthetic-only for `0.1.0` |
| Owner/private and participant/shared named lookup | Verified live on 2026-09-14: independent exact lookups of `WortJagdFamily` / `inventory` observed matching `WJInventory` type, change tag, creation time, and modification time | Additional entry-record evidence is optional and requires an operator-supplied exact non-content identifier |
...
| Secondary diagnostic adversarial contracts | Synthetic coverage validates strict change/share/subscription shapes, typed cyclic cursor rejection, compact drill-down zone handles, cursor expiry/type/context failures, tombstones and partials, custom-zone enforcement, exact share attribution without fallbacks, private topology omission, subscription error separation, pre-credential capability gates, MCP cancellation before and after dispatch, independent zone correspondence, failed-side comparison handling, same-principal limits, and non-transactional observation windows | Retain live `get_share` unavailability and shared subscription/list-zone gates unless separate live evidence proves more |
...
| User credential import and storage | Owner and participant sessions were separately imported, principal-bound, and repeatedly rotated during live reads on 2026-09-14 without entering uncertain state; restarted installed-MCP probes repeated owner/private and participant/shared access with certain rotation on 2026-09-14; synthetic account-switch and expiry coverage fails closed until explicit reauthentication | Normal owner/participant operation is live verified; deliberate live account-switch and expiry remain human-gated `0.1.0` limitations |
| Query/date/number/error wire contracts | Adversarial synthetic coverage validates bounded typed scalar and homogeneous `IN` filters, precision-loss rejection, exact wire dictionaries, strict record collections, identity-correlated lookup, empty partial pages, context-bound markers with cycle detection and full-page registry pressure, query-lag versus exact-lookup semantics, privacy-safe top-level and per-item errors, malformed dates and record identities, numeric-string preservation, explicit nested identity/asset redaction, unexpected-field omission, and aggregate projection exhaustion | Retained as `notVerified` for `0.1.0`: the acceptance profiles authorize no record type or query fields and no privacy-safe selector was supplied; synthetic behavior is not Apple integration evidence |
| Release-acceptance profile payload policy | Metadata-only by operator choice: allowed record types, queryable fields, and readable payload fields are empty | No selector was inferred; zero-filter, filtered-query, and payload-read live verification remain outside current acceptance evidence |
...
| Release-candidate core live acceptance | Two-account core read acceptance run on 2026-09-14 with CloudKit Web Services v1 and Node 25: identity probes, private zone list/lookup, shared database/zone changes, matching canonical named-record reads, empty-field projection, and owner subscription listing passed; no mutations | Complete for the enabled metadata-only surface; query, server-key, deliberate account-switch/expiry, private change-feed, generic shared-zone, shared-subscription, and share-topology limitations remain explicit |
| Public contract reconciliation | README, tool table, packaged capability resource, operation policy, provenance, runtime tool registry, and high-risk MCP input schemas are cross-checked; the accepted metadata-only surface and retained live versus synthetic limitations are explicit | Reconcile again only if later live evidence or release execution changes these claims |
